...
def battery_cost_A(x):
    p, contain = x
    data = pd.read_csv('问题1：（1）A.csv')
    data = data.dropna()
...
def battery_cost_C(x):
    p, contain = x
    data = pd.read_csv('问题1：（1）C.csv')
    data = data.dropna()
...
def main():
    func_name = [battery_cost_A, battery_cost_B, battery_cost_C]
    for fun in func_name:
        # PSO粒子群优化，优化储能系统的功率和容量配置
        # 调整PSO的参数，以增强粒子的探索能力
        pso = PSO(func=fun, dim=2, pop=100, max_iter=200, lb=[0, 0], ub=[100, 1000], w=0.9, c1=0.8, c2=0.8)
        pso.run()
